...
def github_token() -> str:
    """Recupera el token de GitHub desde la variable de entorno GITHUB_TOKEN."""
...
def pull_repository(context: OpExecutionContext, configure_git: str, github_token: str) -> str:
    ...
    if result.returncode != 0:
        raise RuntimeError(f"git pull: {result.stderr.strip()}")
    ...
    if result.returncode != 0:
        raise RuntimeError(f"git checkout: {result.stderr.strip()}")
