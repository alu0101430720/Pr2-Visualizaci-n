...
@asset(deps=["limpiar_codislas"])
def guardar_codislas_limpio(context: OpExecutionContext, limpiar_codislas: pd.DataFrame) -> str:
    """Guarda df_codislas_clean en datasets-clean/."""
    directorio = "Pr2-Visualizacion/datasets-clean"
    os.makedirs(directorio, exist_ok=True)
    ruta = directorio + "/codislas-clean.csv"
    limpiar_codislas.to_csv(ruta, index=False)
    context.log.info("codislas-clean guardado en: " + ruta)
    return ruta


@asset(deps=["construir_ejercicio2"])
def guardar_ejercicio2(context: OpExecutionContext, construir_ejercicio2: pd.DataFrame) -> str:
    """Guarda df_ejercicio2 en datasets-clean/."""
    directorio = "Pr2-Visualizacion/datasets-clean"
    os.makedirs(directorio, exist_ok=True)
    ruta = directorio + "/ejercicio2.csv"
    construir_ejercicio2.to_csv(ruta, index=False)
    context.log.info("ejercicio2.csv guardado en: " + ruta)
    return ruta
...
def generar_grafico_ejercicio2(context: OpExecutionContext, construir_ejercicio2: pd.DataFrame) -> str:
    ...
    grafico = graficar_renta_territorial(
    ...
    if grafico is None:
        raise RuntimeError("El gráfico no se generó: datos vacíos para " + territorio)

    nombre_archivo = "ejercicio2_renta_" + territorio.lower().replace(' ', '_') + ".png"
    ruta_grafico = directorio + "/" + nombre_archivo
    grafico.save(ruta_grafico, dpi=150)
    context.log.info("Gráfico ejercicio2 guardado en: " + ruta_grafico)
    return ruta_grafico


@asset(deps=["guardar_codislas_limpio", "guardar_ejercicio2", "generar_grafico_ejercicio2", "github_token"])
def commit_ejercicio2(
    context: OpExecutionContext,
    github_token: str,
    guardar_codislas_limpio: str,
    guardar_ejercicio2: str,
    generar_grafico_ejercicio2: str,
) -> None:
    """Hace add, commit y push de los archivos generados en el ejercicio 2."""
    repo_dir = "Pr2-Visualizacion"
    repo_url = "https://" + github_token + "@github.com/alu0101430720/Pr2-Visualizacion.git"

    for archivo in [guardar_codislas_limpio, guardar_ejercicio2, generar_grafico_ejercicio2]:
    ...
    result = subprocess.run(
        ["git", "-C", repo_dir, "commit", "-m", "ejercicio2: codislas, df integrado y grafico territorial"],
        capture_output=True, text=True
    )
    if result.returncode != 0:
